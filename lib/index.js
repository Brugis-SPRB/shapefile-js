...
shp.parsePrj = function(prjWKT) {
  /* HACK To override PRJ definition in case of Lambert72 because PRJ WKT does not contains toWGS conversion parameters */
  /* Belge_1972_Belgian_Lambert_72  or Belge 1972 / Belgian Lambert or ...*/
  var lambert72Pattern = /(Belge.1972)+/g;
  if ( lambert72Pattern.test(prjWKT) ) {
    return proj4("+proj=lcc +lat_1=51.16666723333333 +lat_2=49.8333339 +lat_0=90 +lon_0=4.367486666666666 +x_0=150000.013 +y_0=5400088.438 +ellps=intl +towgs84=-106.869,52.2978,-103.724,0.3366,-0.457,1.8422,-1.2747 +units=m +no_defs");
  } else {
    return proj4(prjWKT);
  }
};
...
shp.parseZip = function(buffer, whiteList) {
  ...
  for (key in zip) {
    ...
    if (key.slice(-3).toLowerCase() === 'shp') {
      ...
      zip[key.slice(0, -3) + key.slice(-3).toLowerCase()] = shp.parsePrj(zip[key]);
